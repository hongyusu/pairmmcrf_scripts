...
function rtn = run_pairMMCRF(inname)
...
pa=[mmcrf_cs;selRes];
pa=[[0,mmcrf_gs]',pa]
dlmwrite(sprintf('../parameters/%s_parammcrfpair',name{1}),pa)
...
[acc,vecacc,pre,rec,f1,auc1,auc2]=get_performance(Y,YenspredBin>=0);
perf=[perf;[acc,vecacc,pre,rec,f1,auc1,auc2]];perf
dlmwrite(sprintf('../predictions/%s_predBinPairEns',name{1}),YenspredBin>=0)
% performance of Val ensemble
[acc,vecacc,pre,rec,f1,auc1,auc2]=get_performance(Y,Yenspred,YenspredVal);
perf=[perf;[acc,vecacc,pre,rec,f1,auc1,auc2]];perf
dlmwrite(sprintf('../predictions/%s_predValPairEns',name{1}),Yenspred)
...
[acc,vecacc,pre,rec,f1,auc1,auc2]=get_performance(Y,Ypred>=0,Ypred);
perf=[perf;[acc,vecacc,pre,rec,f1,auc1,auc2]];perf
dlmwrite(sprintf('../predictions/%s_predMadPairEns',name{1}),Ypred>=0)
...
print(hFig, '-depsc',sprintf('../plots/%s_PairEns.eps',name{1}));
% save results
dlmwrite(sprintf('../results/%s_perfPairEns',name{1}),perf)
dlmwrite(sprintf('../results/%s_perfRandPairEns',name{1}),perfRand)
dlmwrite(sprintf('../results/%s_perfValPairEnsProc',name{1}),perfValEns)
dlmwrite(sprintf('../results/%s_perfBinPairEnsProc',name{1}),perfBinEns)
dlmwrite(sprintf('../results/%s_perfMadPairEnsProc',name{1}),perfMadEns)
